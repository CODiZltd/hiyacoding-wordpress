jQuery(function($) {
    ...
    var seoGrow = window.seoGrow || {};

    seoGrow.mainFunction = function() {
        if ($('.back-to-top').length) {
            ...
            $('.back-to-top').on('click', function(e) {
                e.preventDefault();
                $('.back-to-top').addClass('run');
                $('.back-to-top').addClass('hidden-mark');
                setTimeout(function() {
                    $('html,body').animate({
                        scrollTop: 0
                    }, 1000);
                ...
                setTimeout(function() {
                    $('.back-to-top').removeClass('hidden-mark');
                    $('.back-to-top').removeClass('run');
                },1500)
            });
        }
        ...
        //Service tab
            if($(window).width() < 768) {
                if ($('.sc-service-tab').length) {
                    $('.sc-service-tab .tab-content').slick({
                      slidesToShow: 1,
                      slidesToScroll: 1,
                      arrows: false,
                      fade: true,
                      asNavFor: '.sc-service-tab .tab-list',
                      infinite: false,
                    });
                    $('.sc-service-tab .tab-list').slick({
                      slidesToShow: 1,
                      slidesToScroll: 1,
                      asNavFor: '.sc-service-tab .tab-content',
                      dots: false,
                      arrows: true,
                      // centerMode: true,
                      focusOnSelect: true,
            ...
            window.onresize = function(event) {
                if ($(window).width() < 768) {
                    ...
                    if ($('.sc-service-tab').length) {
                        $('.sc-service-tab .tab-content').slick('unslick');
                        $('.sc-service-tab .tab-list').slick('unslick');
                    }
                }
            }; 
    ...
    seoGrow.headerFunction = function() {
        ...
        $('body').on('click', function (event) {
            if ($('.slz-button-search').has(event.target).length == 0 && !$('.slz-button-search').is(event.target) && $('.nav-search').has(event.target).length == 0 && !$('.nav-search').is(event.target)) {
                if ($('.nav-wrapper').hasClass('hide') == false) {
                    ...
                    $('.slz-button-search').removeClass('active');
                    $('.nav-search-full').removeClass('active');
                }
            }
        });
    ...
    $(document).ready(function() {
        seoGrow.mainFunction();
        seoGrow.headerFunction();
