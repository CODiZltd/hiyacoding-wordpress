...
if ( function_exists( 'seogrow_implement_options' ) == false ){
	...
	function seogrow_implement_options() {
        ...
        //related product
        $show_related_product = slz_get_db_settings_option('product-related-post');
        if(!empty($show_related_product['status']) && $show_related_product['status'] == 'hide'){
        	$custom_css .= '.related.products {display:none;}';
        }

		// layout
		if ( isset( $settings['layout-group'] ) && $layout_group = $settings['layout-group'] ) {
			if ( isset($layout_group['layout']) && $layout_group['layout'] == 'boxed'){
				add_filter( 'body_class','seogrow_add_body_class_boxed' );
		...
		//site color
		if( $typo = seogrow_setting_theme_color($settings) ) {
			$custom_css .= $typo;
			$is_change_color = true;
		}
		//typography
		if( $typo = seogrow_setting_typography($settings) ) {
			$custom_css .= $typo;
			$is_change_color = true;
		}
		if( $is_change_color ) {
			add_filter( 'body_class','seogrow_add_body_class_custom_color' );
		}
		//--------- >> Change color & typography----------------
...
if ( function_exists( 'seogrow_setting_typography' ) == false ):
	function seogrow_setting_typography( $settings ){
...
if ( function_exists( 'seogrow_setting_theme_color' ) == false ):
	function seogrow_setting_theme_color( $settings ){
...
if ( function_exists( 'seogrow_add_body_class_custom_color' ) == false ){

	function seogrow_add_body_class_custom_color( $classes ) {
