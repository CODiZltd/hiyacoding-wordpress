	...
	if ( !empty( $items ) ) {
		...
		foreach ($items as $item) {
			...
			if(!empty($item['block_bg_color'])){
				$css = '
						.%1$s .icon-box-item-%2$s.has-background {
			...
			if(!empty($item['block_bg_hv_color'])){
				$css = '
						.%1$s .icon-box-item-%2$s.has-background:hover {
							background-color: %3$s;
						}
					';
			...
			if ( !empty( $item['button_text_hv_color'] ) ) {
				$css = '
					.%1$s .icon-box-button-%2$s.slz-btn:hover {
						color: %3$s;
					}
					.%1$s .icon-box-button-%2$s.slz-btn::before {
						background: %3$s;
					}
				';
				$custom_css .= sprintf( $css, esc_attr( $data['block_class'] ), esc_attr( $i ), esc_attr( $item['button_text_hv_color'] ) );
			}
			...
			$out .= '<div class="item">';
				...
				$out .= '<div class="slz-icon-box-1 icon-box-item-'.esc_attr($i).' '.esc_attr( $class_view ).' '. esc_attr( $align ) .' '. $class_background .'">';
