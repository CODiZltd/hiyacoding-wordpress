...
for ($i=1; $i <= $data['column'] ; $i++) {
    ...
    $out .= '<div class="item">';
        $out .= '<div class="slz-pricing-table-01 pricing-box-total-'.esc_attr( $i ).' '.esc_attr( $active ).' ' .esc_attr( $class_type ).'">';
            ...
            if ($data['style'] == '1') {
                ...
                $out .= '<div class="pricing-header-2">';
                    if ( !empty( $data['title'.$i] ) ) {
                        $out .= '<div class="title pricing-box-title-'.esc_attr( $i ).'">'.esc_html( $data['title'.$i] ).'</div>';
                    }
                    if ( !empty( $data['feature_list'.$i] ) && $data['style'] == '2')  {
                        if (function_exists('vc_param_group_parse_atts')) {
                            $items = (array)vc_param_group_parse_atts($data['feature_list'.$i]);
                            foreach ($items as $item) {
                                $item = array_merge( $param_default, $item );
                                if ( $item['icon_type'.$i] == '2' ) {
                                    if ( !empty( $item['image'.$i] ) && $img_url = wp_get_attachment_url( $item['image'.$i] ) ) {
                                        $out .= '
                                                        <div class="wrapper-icon wrapper-icon-image">
                                    ...
                                    $icon = $item['icon_library' . $i];
                                    if (!empty($item['icon_' . $icon . $i])) {
                                        SLZ_Util::slz_icon_fonts_enqueue($item['icon_library' . $i]);
                                        $out .= '
                                                        <div class="wrapper-icon wrapper-icon'.esc_attr( $i ).'">
                                                            <i class="slz-icon ' . esc_attr($item['icon_' . $icon . $i]) . '"></i>
                                                        </div>
                                                        ';
                                    }
                                    if ( !empty( $item['icon_color'.$i] ) ) {
                                        $css = '
                                                        .%1$s .wrapper-icon'.$i.' i {
                                                            color: %3$s;
                                                        }
                                                    ';
                                        $custom_css .= sprintf( $css, esc_attr( $block_class ), esc_attr( $i ), esc_attr( $item['icon_color'.$i] ) );
                                    }
                                }
                            }
                        }
                    }
                    $out .= '<div class="pricing-section-2 pricing-box-section-'. esc_attr( $i ).'">';
                        if ( !empty( $data['price'.$i] ) || !empty( $data['currency'.$i] ) ) {
                            if ( !empty( $data['separate'.$i] ) ) {
                                $separator = $data['separate'.$i];
                            }else{
            ...
            $out .= '<div class="pricing-body pricing-box-pricing-body-'.esc_attr( $i ).'">';
                ...
                if ( !empty( $pricing_option_arr ) ) {
                    ...
                    foreach ($pricing_option_arr as $pricing_option) {
                        ...
                        if ($data['style'] == '2') {
                            $out .= '
                                    <div class="pricing-option pricing-box-option-'.esc_attr( $x ).'">
                                        <i class="slz-icon ' . esc_attr($pricing_option['icon_feature' . $i]) . '"></i>
                                        <span class="text"> ' . esc_html( $pricing_option['text'] ) . ' </span>
                                    </div>';
                            if (!empty($pricing_option['pricing_options_color_icon'])) {
                                $css = '
                                    .%1$s .pricing-box-pricing-body-%2$s .pricing-option.pricing-box-option-%4$s i {
                                        color: %3$s;
                                    }
                                ';
                                $custom_css .= sprintf($css, esc_attr($block_class), esc_attr($i), esc_attr($pricing_option['pricing_options_color_icon']), esc_attr($x));
                            }
	...
	if ( !empty( $data['title_color'.$i] ) ) {
		$css = '
			.%1$s .pricing-box-total-%2$s .active .pricing-header {
				color: %3$s;
			}
			.%1$s .pricing-box-total-%2$s .pricing-header .title {
				color: %3$s;
			}
		';
		$custom_css .= sprintf( $css, esc_attr( $block_class ), esc_attr( $i ), esc_attr( $data['title_color'.$i] ) );
	}
	if ( !empty( $data['title_color_bg'.$i] ) ) {
		$css = '
			.%1$s .pricing-box-total-%2$s .active .pricing-header {
				background-color: %3$s;
			}
			.%1$s .pricing-box-total-%2$s .title {
				background-color: %3$s;
			}
		';

		$custom_css .= sprintf( $css, esc_attr( $block_class ), esc_attr( $i ), esc_attr( $data['title_color_bg'.$i] ) );
	}
	...
	if ( !empty( $data['btn_text_color'.$i] ) ) {
		$css = '
			.%1$s .slz-pricing-table-01 .pricing-footer .pricing-box-button-%2$s  {
				color: %3$s;
			}
		';
    ...
    if ( !empty( $data['btn_background_color_hv'.$i] ) ) {
        $css = '
			.%1$s .slz-pricing-table-01 .pricing-footer .pricing-box-button-%2$s:before {
				background-color: %3$s;
			}
		';
        $custom_css .= sprintf( $css, esc_attr( $block_class ), esc_attr( $i ), esc_attr( $data['btn_background_color_hv'.$i] ) );
    }
	...
	if ( !empty( $data['label_background_color'.$i] ) ) {
		$css = '
			.%1$s .slz-pricing-table-01 .pricing-box-label-%2$s {
				background-color: %3$s;
			}
		';
...
if ( is_plugin_active( 'js_composer/js_composer.php' ) ) {
	...
	echo esc_html__('Please Active Visual Composer', 'seogrow');
}
