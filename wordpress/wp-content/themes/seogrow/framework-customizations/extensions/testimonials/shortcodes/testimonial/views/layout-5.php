...
if( $data['style'] == '2' ){
	...
	$html_format = '
	<div class="item">
		<div class=" testimonial-item ' .$class_style.' ">
...
$html_render['title_format'] = '<div class="name">%2$s</div>';
$html_render['title_format'] = '<div class="name">%1$s</div>';
$html_render['position_format'] = '<a class="position">%1$s</a>';
?>
<div class=" slz-testimonial slz-testimonial-02 layout-05 slz-shortcode <?php echo esc_attr($data['block_cls']);?>"
    ...
    data-slidestoshow="<?php echo esc_attr( $data['num_column_show'] ); ?>"
...
if ( !empty( $data['arrows_color'] ) ) {
	$css = '
		.%1$s .btn-prev i:before,
		.%1$s .btn-next i:before{
			color: %2$s;
		}
	';
...
if ( !empty( $data['arrows_hv_color'] ) ) {
	$css = '
		.%1$s .btn-prev:hover i:before,
		.%1$s .btn-next:hover i:before{
			color: %2$s;
		}
	';
...
if ( !empty( $data['line_color'] ) ) {
	$css = '
		.%1$s.slz-testimonial-02.layout-05 .testimonial-item.style-2 .testimonial-wrapper .description{
			border-bottom: 1px solid %2$s;
		}
	';
	$custom_css .= sprintf( $css, esc_attr( $data['uniq_id'] ), esc_attr( $data['line_color'] ) );
}
