...
echo '<div class="slz-shortcode slz-shortcode-list sc_info_box '. esc_attr( $block_cls ).' '.esc_attr($data['styles']).'">';
if( isset( $data['styles'] ) ) {
    if( $data['styles'] == 'style-1' ) {
        if ( !empty( $data['feature_list'] ) ) {
            ...
            if( !empty( $item_arr ) ) {
                echo '<div class="slz-list-block slz-info-box slz-list-column '.esc_attr($class_column).'">';
    ...
    else if( $data['styles'] == 'style-3' ) {
        if (!empty($data['information'])) {
            echo '<div class="slz-list-block slz-info-box">';
            $item = array(
                'information' => $data['information'],
                'show_icon' => $data['show_icon'],
                'image' => $data['image'],
                'icon_fontawesome' => $data['icon_fontawesome']
            );
            $item = array_merge($default_params, $item);

            echo slz_render_view($instance->locate_path('/views/style-3.php'), compact('item'));
            echo '</div>';
        }
    }
}
...
if( !empty( $data['text_color'] ) ) {
	$css = '
		.%1$s.sc_info_box .slz-icon-box-1.style-vertical .wrapper-info .title {
			color: %2$s;
		}
	';
...
if( isset( $data['des_color'] ) && !empty( $data['des_color'] ) ) {
    $css = '
		.%1$s.sc_info_box .slz-icon-box-1.style-vertical .wrapper-info .description {
			color: %2$s;
		}
	';
...
if( isset( $data['number_color'] ) && !empty( $data['number_color'] ) ) {
    $css = '
		.%1$s.sc_info_box .slz-icon-box-1.style-vertical .number {
			color: %2$s;
		}
	';
...
if( !empty( $data['border_color'] ) ) {
    $css = '
		.%1$s.sc_info_box .slz-info-box > .item::before {
			background-color: %2$s;
		}
	';
...
if( !empty( $data['icon_color'] ) ) {
    $css = '
		.%1$s.sc_info_box .slz-icon-box-1.style-vertical .icon-cell {
			color: %2$s;
		}
	';
