    ...
    ?>
        ...
        if( !empty( $data['array_info'] ) ) {
            ...
            for ($j = $x; $j < ($column + $x) && $j < count($data['array_info']); $j++) {
                ...
                echo '<div class="item"><div class="slz-contact-01">';
                ...
                if (!empty($info['array_info_item'])) {
                    ...
                    foreach ($info['array_info_item'] as $item) {
                        ...
                        if( !empty( $item['title'] ) ) {
                            ...
                            echo '<div class="text">' . wp_kses_post( nl2br( $item['title'] ) ) . '</div>';
                            echo '</div>';
                        }
                    }
                }
                ...
                if (!empty($info['array_sub_info_item'])) {
                    ...
                    foreach ($info['array_sub_info_item'] as $item) {
                        ...
                        if( !empty( $item['sub_info'] ) ) {
                            echo '<div>';
                            $format = '<i class="slz-icon %1$s"></i>';
                            echo ( $shortcode->get_icon_library_views( $item, $format ) );
                            echo '<div class="text">'.wp_kses_post( nl2br( $item['sub_info'] ) ).'</div>';
                            echo '</div>';
                        }
                    }
                }
