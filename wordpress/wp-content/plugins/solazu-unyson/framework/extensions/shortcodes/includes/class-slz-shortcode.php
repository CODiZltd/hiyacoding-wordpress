...
class SLZ_Shortcode
{
	...
	public function get_icon_library_options( $dependency = array() ){
		$results = array();
	...
	public function get_icon_library_views( $item = array(), $format = '' ){
		$out = '';
		if( empty( $item ) ){
			return $out;
		}
		...
		$icon = 'vs';
		if( isset( $item['icon_library'] ) && !empty( $item['icon_library'] ) ){
