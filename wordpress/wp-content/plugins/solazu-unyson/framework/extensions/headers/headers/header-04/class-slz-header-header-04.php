		...
		if ( !empty ( $options ) ) {
			...
			if ( !empty ( $topbar ) && $topbar['selected-value'] == 'yes' ){
				...
				if ( !empty ( $topbar_style ) ){
					...
					if ( !empty ( $social_color ) ) {
						...
						$custom_css .= '.slz-topbar-list .social i { color: ' . esc_attr( $social_color ) . ' }';
			...
			if( !empty ( $options['enable-search'] ) && $options['enable-search'] == 'yes' ){
				...
				if ( !empty ( $search_style ) ) {
					...
					$bg_color = SLZ_Com::get_palette_selected_color( $search_style['text-color'] );

					if ( !empty ( $bg_color ) ){

						$custom_css .= 'header .nav-search form input[type=\'text\'] { color: ' . esc_attr( $bg_color ) . ' }';
