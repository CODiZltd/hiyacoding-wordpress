...
class SLZ_Minify_System {
	...
	public function __construct( ) {
		...
		$this->cache_location = slz_get_framework_directory('/cache');
	...
	function add_styles($todo) {
		...
		if( !empty( $not_minify ) ) {
			foreach ( $not_minify as $item ) {
				...
				$this->css_tags .= "<link rel='stylesheet' id='". $item['handle'] ."'  href='". $item['url'] ."' type='text/css' media='all' />";
			}
		}
	...
	function add_scripts($todo) {
		...
		if( isset( $wp_scripts->registered['slz-slz-ajax-form'] ) ) {
			$ajax_form = ( array ) $wp_scripts->registered['slz-slz-ajax-form'];
			if( isset( $ajax_form['extra']['data'] ) ) {
				$this->js_tags .= '<script type="text/javascript" >'. $ajax_form['extra']['data'] .'</script>';
			}
		}
		...
		if( !empty( $not_minify ) ) {
			foreach ( $not_minify as $item ) {
				...
				$this->js_tags .= '<script type="text/javascript" src="'. $item .'" ></script>';
			}
		}
		...
		foreach ($minify_urls_footer as $minify_url) {
			$this->js_tags .= "<script type='text/javascript' src='$minify_url'></script>";
		}
	...
	function fetch_content($url, $type = '') {
		$cache_file = slz_get_framework_directory('/cache/'.md5($url).$type);
		$content = '';
	...
	function get_cached_minify_urls($urls, $type) {
		...
		foreach ($urls as $url) {
			...
			if (file_exists($cache_file)) {
				$cache_url = slz_get_framework_directory_uri() . '/cache/'.md5($url).$type.'?m='.filemtime($cache_file);
